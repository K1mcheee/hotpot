...
Do not decompile the test classes, you can directly run them. Decompiling may cause issues with testing.
...
```
There is also a `Pair` and `Ingredient` class that should not be modified.
...
simulating chopping each ingredient into `n` pieces. We assume input `n` will always be more than 1 as we are not operating a 
quantum hotpot restraunt.
...
any repeats. Implement a method that removes any duplicates.
...
You can test your implementation by running:
```shell
java -cp hotpot-test.jar:. Test6
```
Extra: `List<Pair<Ingredient, Double>> calculate`

You decided to implement a auto-calculation system that takes in 2 parameters:

- A bowl of ingredients `List<Pair<Pair<Ingredient,String>, Double>>` representing a list of a 
((ingredient, category) pair , weight) pair. 
- A list of prices ` List<Pair<String, Double>>` representing a (category, price) pair.

Calculate the total price of each ingredient in the bowl and return it as a list of (ingredient, price) pair. 
If there is a missing price stated for a category, assume it costs 0.0. Unfortunately this means you lose money :(

Sample output:
```java
jshell> Ingredient cabbage = new Ingredient("Cabbage", true);
jshell> Ingredient beef = new Ingredient("Beef", true);
jshell> List<Pair<String, Double>> prices = List.of(
   ...>     new Pair<>("Vegetable", 2.0),
   ...>     new Pair<>("Meat", 10.0)
   ...> );
jshell> List<Pair<Pair<Ingredient, String>, Double>> bowl1 = List.of(
   ...>     new Pair<>(new Pair<>(cabbage, "Vegetable"), 2.5),
   ...>     new Pair<>(new Pair<>(beef, "Meat"), 1.0)
   ...> );
jshell> Hotpot.calculate(bowl1, prices).get(0).getSecond()
$.. ==> 5.0
jshell> Hotpot.calculate(bowl1, prices).get(1).getSecond()
$.. ==> 10.0

jshell> List<Pair<Pair<Ingredient, String>, Double>> bowl2 = List.of(
   ...>     new Pair<>(new Pair<>(beef, "Premium"), 2.0)
   ...> );
jshell> Hotpot.calculate(bowl2, prices).get(0).getSecond()
$.. ==> 0.0
```
You can test your implementation by running:
...
There are no testcase for that but you can use the code provided to see expected output by running
